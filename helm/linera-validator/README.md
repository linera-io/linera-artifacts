...
| prometheusRule.enabled | bool | `false` | Create a PrometheusRule with the default Linera alerts (validator down, shard restart, proxy high latency). Extend with extraRules below. OPT-IN: requires prometheus-operator + the PrometheusRule CRD installed in the cluster. |
